...
export default function VuaTiengVietGame({ initialQuestions, onBack }: VuaTiengVietProps) {
  ...
  const [wrongAnswer, setWrongAnswer] = useState<string | null>(null); // show correct answer when wrong
  const [isChecking, setIsChecking] = useState(false); // prevent double-submit
  ...
  const checkAnswer = () => {
    ...
    if (userClean === correctClean) {
      ...
      setIsChecking(true);
      sounds.wrong();
      setWrongAnswer(q.answer); // show correct answer
      showFeedback('SAI RỒI!', 'error');
      nextQuestion(); // automatically move to next after delay
    }
  };
          ...
          <div className="flex-1 flex flex-col md:flex-row gap-8 items-center">
              ...
              <div className={cn("flex flex-col space-y-6 w-full", q.image ? "md:w-1/2" : "max-w-2xl mx-auto")}>
                  <div className="text-2xl font-bold text-gray-800 text-center leading-tight">
                    <Markdown remarkPlugins={[remarkMath]} rehypePlugins={[rehypeKatex]} components={{ p: ({children: c}) => <>{c}</> }}>{q.text}</Markdown>
                  </div>
                  ...
                  <div className="space-y-4">
                      {wrongAnswer && (
                        <div className="bg-green-50 border-2 border-green-300 rounded-2xl px-5 py-3 text-center animate-pulse">
                          <p className="text-xs uppercase font-black text-green-500 mb-1">Đáp án đúng</p>
                          <p className="text-xl font-black text-green-700 uppercase tracking-widest">{wrongAnswer}</p>
                        </div>
                      )}
                      <input 
                        type="text" 
                        ...
                        className="w-full px-6 py-4 rounded-2xl border-2 border-gray-200 focus:border-red-400 outline-none text-2xl font-bold uppercase text-center placeholder:text-gray-300 transition-colors disabled:opacity-50 disabled:bg-gray-50"
                      />
                      <button 
                        onClick={checkAnswer}
                        disabled={isChecking}
                        className="w-full bg-gradient-to-r from-[#FF512F] to-[#DD2476] text-white py-4 rounded-2xl font-black text-xl uppercase tracking-widest shadow-lg hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                          {isChecking ? 'ĐANG CHUYỂN...' : 'KIỂM TRA'}
                      </button>
                  </div>
              </div>
          </div>
